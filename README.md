...
Реализовано: ingestion API с Detection Engine (13 правил, MITRE ATT&CK),
...
версии под той же лицензией. Это не позволяет тихо превратить проект в закрытый SaaS-конкурент без
раскрытия исходников, но не запрещает ни коммерческое использование, ни закрытый внутренний форк
для собственных нужд без предоставления доступа третьим лицам как к сетевому сервису.
...
- <http://localhost:8000/docs> — только при `WG_APP_ENV=development`; в production OpenAPI-схема
  и `/docs` отключены (см. `main.py`)
...
Агрегатный отчёт за семь суток печатается в stdout; файл создаёт оператор перенаправлением:
...
Период — `[начало, конец)` в UTC, поэтому два соседних отчёта не считают одно и то же дважды.
Инциденты относятся к периоду по моменту открытия, события — по моменту на хосте.

**PDF получается печатью из HTML** (`Ctrl+P` → «Сохранить как PDF»), а не генерируется кодом:
библиотека PDF потребовала бы новой зависимости и встроенного TTF-шрифта с кириллицей в
репозитории, тогда как браузер делает это лучше и бесплатно
([ADR-0023](docs/adr/0023-weekly-report.md)).

В отчёт входят только счётчики и группировки. **IP-адресов в нём нет намеренно**: отчёт может
уходить заказчику, а адрес легитимного пользователя с динамическим IP — персональные данные.
Каждый отчёт печатает собственные ограничения: счётчики отражают доставленное агентом, а
отсутствие инцидентов не означает отсутствия атак.

## Roadmap

Ниже — не список обещаний, а честная сводка того, что уже решено не делать, и того, что
отложено до появления реальной потребности. Источник каждого пункта — ADR или раздел
«Ограничения MVP» ниже; здесь они просто собраны в одном месте.

**Отклонено окончательно** (решение принято, обоснование не устаревает от нехватки времени):

- исполнение блокировки IP агентом или control plane — [ADR-0016](docs/adr/0016-no-automated-block-execution.md);
- источник событий Docker (`docker events`) — потребовал бы доступа к Docker-сокету, то есть
  root на хосте — [ADR-0021](docs/adr/0021-no-docker-events-source.md).

**Отложено осознанно** (реализация ясна, но цена пока не оправдана объёмом использования):

- генерация PDF отдельной библиотекой вместо печати из HTML — потребовала бы новой зависимости
  и встроенного кириллического шрифта — [ADR-0023](docs/adr/0023-weekly-report.md);
- обнаружение новых файлов в наблюдаемом каталоге (сейчас — только явно перечисленные пути) —
  [ADR-0022](docs/adr/0022-file-integrity-source.md);
- чтение `error.log` и произвольного `log_format` Nginx — [ADR-0020](docs/adr/0020-nginx-access-log-source.md);
- RFC5424 и уже сжатые ротированные файлы (`auth.log.1.gz`) для `syslog_file` — [ADR-0019](docs/adr/0019-syslog-file-source.md);
- добавление комментария к инциденту через Telegram (сейчас — только через Dashboard);
- HTTP API управления серверами и ключами агентов (сейчас — только локальный CLI);
- отслеживание и автоматизация снятия блокировки IP;
- координация rate limiter между несколькими экземплярами control plane;
- автоматическая ротация ключей и очистка старых событий;
- проверка актуальности применённой миграции в `/health/ready` (сейчас проверяется только
  соединение с PostgreSQL).

Полный перечень технических ограничений текущего MVP — ниже.
...
- [docs/weekly-report.md](docs/weekly-report.md) — еженедельный отчёт, его границы и что в него
  намеренно не входит;
