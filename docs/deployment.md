...
Woland Guard позволяет предлагать и подтверждать план блокировки, но **никогда не исполняет его
сам** — команда только показывается оператору, применяет её человек. Это окончательное решение
(ADR-0016), а не временное ограничение.

Чтобы показанная команда была применима, на защищаемом сервере должны заранее существовать
таблица и оба set, которые использует `nft add element`:
...
`WG_IP_BLOCK_NFT_SET_V6` и должны совпадать с тем, что создано на сервере вручную — изменение
firewall хоста без ведома администратора не входит в задачи проекта.

Правила выше сбрасывают трафик с адресов из set, но **не снимают блокировку автоматически**:
удаление элемента (`nft delete element ...`) остаётся ручной операцией, которую проект не
отслеживает. Планируйте это заранее, иначе набор заблокированных адресов со временем станет
состоянием, о котором никто не знает.
