...
- `linux.file.changed` — атрибуты `path` (настроенный оператором), `change`
  (`content`/`permissions`/`metadata`/`appeared`/`disappeared`) и `monitoring`
  (`content` — сравнивалось содержимое, `metadata` — файл недоступен на чтение и сравнивались
  только размер, время и права). Ни содержимого, ни diff, ни хэша событие не несёт (ADR-0022)
...
| `system_file_changed` | single | high | — |
...
`system_file_changed` оставлен без `mitre_attack_ids` по той же причине, что и
`nginx_error_spike`: техника зависит от того, какой именно файл изменился и как — правка
`sshd_config` (T1556) и смена прав (T1222) относятся к разным техникам, а правило работает над
произвольным настроенным списком, поэтому единая привязка была бы ложной атрибуцией.
