...
локально распознаёт только закрытый набор классов событий и не пересылает `MESSAGE`, environment или
...
- Контроль целостности файлов (`file_integrity`) не запрашивает прав. Файлы, доступные агенту на
  чтение, сравниваются по содержимому; недоступные (`/etc/sudoers`, `/etc/shadow`) — только по
  размеру, времени и правам. **Второй режим обходится** восстановлением mtime через `touch -r`,
  поэтому отсутствие события в нём ничего не доказывает (ADR-0022 §2).
- `/etc/shadow` не включён в пример конфигурации намеренно: он меняется при каждой смене пароля
  любым пользователем, поэтому на многопользовательском хосте сообщал бы о рутине.
- Изменение, отменённое внутри одного интервала опроса, не будет замечено — это цена опроса
  вместо inotify (ADR-0022 §5).
- Новый файл, положенный в наблюдаемый каталог (например `/etc/cron.d/`), **не обнаруживается**:
  наблюдаются только явно перечисленные пути. Это осознанное ограничение этого этапа
  (ADR-0022 §6).
- Контроль целостности не переживает атакующего с правами root: чтобы изменить `/etc/ssh/sshd_config`,
  нужен root, а с ним можно остановить агент или удалить его spool. Правило рассчитано на
  неосторожное изменение и на атакующего, который до агента не добрался.
...
- «Остановлена критичная служба» (`critical_systemd_unit_stopped`) работает только через
  `source: journald`. Сигнал строится на структурных полях `MESSAGE_ID`/`UNIT`, которые PID1
  пишет напрямую в journald; RFC3164, которым пишет rsyslog в `syslog_file`, таких полей не
  переносит в принципе — на этом источнике правило не сработает никогда, не только реже.
- Формат `MESSAGE_ID`/`UNIT` для этого правила сверен по первоисточнику
  (systemd.journal-fields(7), catalog/systemd.catalog.in), но не проверен на реальном
  запущенном Ubuntu-хосте — как и Nginx-источник, только на синтетических данных.
